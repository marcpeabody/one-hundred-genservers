defmodule TetrexSinglePlayerGameServer do
  use GenServer
  ...
  def init(user_id: user_id) do
    {:ok, board_server_pid} = TetrexBoardServer.start_link([])

    # Create a periodic task to move the piece down
    {:ok, periodic_mover_pid} =
      TetrexPeriodic.start_link(
        [
          period_ms: 1000,
          start: false,
          work: fn -> nil end
  ...
  def handle_continue(
    ...
    board_preview = TetrexBoardServer.preview(board_pid)
    ...
    Phoenix.PubSub.broadcast!(TetrexPubSub, pubsub_topic(user_id), game_update)

    {:noreply, game}
  end

  @impl true
  def handle_continue(:init_periodic, %Game{periodic_move_pid: periodic_mover_pid} = game) do
    this_game_server = self()

    # Set the periodic task to move the piece down
    TetrexPeriodic.set_work(periodic_mover_pid, fn ->
  ...
  def handle_call(:board_preview, _from, %Game{board_pid: board} = game) do
    preview = TetrexBoardServer.preview(board)
  ...
  def handle_call(
        :try_move_down,
        _from,
        %Game{board_pid: board, lines_cleared: lines_cleared, periodic_move_pid: periodic} = game
      ) do
    {_move_result, preview, extra_lines_cleared} = TetrexBoardServer.try_move_down(board)
  ...
  def handle_call(:drop, _from, %Game{board_pid: board, lines_cleared: lines_cleared} = game) do
    {preview, extra_lines_cleared} = TetrexBoardServer.drop(board)
  ...
  @impl true
  def handle_call(:try_move_left, _from, %Game{baord_pid: board} = game) do
    {move_result, _preview} = TetrexBoardServer.try_move_left(board)
  ...
  def handle_call(:try_move_right, _from, %Game{board_pid: board} = game) do
    {move_result, _preview} = TetrexBoardServer.try_move_right(board)
    {:reply, move_result, game, {:continue, :publish_state}}
  end
  ...
  @impl true
  def handle_cast({:new_game, true}, %Game{periodic_mover_pid: periodic, board_pid: board} = game) do
  ...
  def handle_cast(:start_game, %Game{periodic_mover_pid: periodic} = game) do
    Periodic.start_timer(periodic)
    {:noreply, %Game{game | status: :playeing}, {:continue, :publish_state}}
  end
  ...
  def handle_cast(:hold, %Game{board_pid: board} = game) do
    TetrexBoardServer.hold(board)
    {:noreply, game, {:continue, :publish_state}}
  end

  @impl true
  def handle_cast(:rotate, %Game{board_pid: board} = game) do
    TetrexBoardServer.rotate(board)
    {:noreply, game, {:continue, :publish_state}}
  end

  # Internal helper functions

  defp game_over(%Game{periodic_move_pid: periodic} = game) do
    Periodic.stop_timer(periodic)
  ...
  # TODO: Move into own behavior module - differs from multi player game
